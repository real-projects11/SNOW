...
export default async function handler(req, res) {
  ...
  // ---------- DIAGNÓSTICO ----------
  // /api/vote?type=debug  → te dice qué variables ve (sin exponer valores)
  if (req.method === "GET" && req.query.type === "debug") {
    const found = Object.keys(process.env).filter(k =>
      /KV_|UPSTASH|REDIS|TELEGRAM/i.test(k)
    );
    return res.status(200).json({
      variables_encontradas: found,
      url_ok: !!R_URL,
      token_ok: !!R_TOKEN
    });
  }
  ...
  if (!R_URL || !R_TOKEN) {
    return res.status(500).json({
      error: "No encuentro las variables de Upstash.",
      probá: "/api/vote?type=debug"
    });
  }
    ...
    if (req.method === "POST") {
      ...
      if (type === "suggestion") {
        const raw = String(body.name || "")
          .replace(/[^a-zA-Z\u00C0-\u017F' -]/g, "").trim().slice(0, 18);
